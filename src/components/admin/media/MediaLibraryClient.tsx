...
import {
  ...
  FolderPlus, Edit2, Check, FolderOpen, ChevronDown, ChevronRight, FolderPlus as SubfolderIcon
} from "lucide-react";
...
export function MediaLibraryClient({
  ...
  // Sync inspector folder value when selectedAsset changes
  useEffect(() => {
    if (selectedAsset) {
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 auto-rows-max">
            {filteredAssets.map((asset) => (
              <div
                ...
                className={cn(
                  "group bg-white rounded-xl border overflow-hidden cursor-pointer transition-all duration-200 hover:shadow-md",
                  selectedAsset?.id === asset.id ? "border-red ring-2 ring-red/10" : "border-gray-200 hover:border-navy"
                )}
              >
            ...
            <form id="asset-form" onSubmit={handleUpdateAsset} className="p-4 space-y-4">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full text-xs font-medium border border-gray-200 rounded-lg px-3 py-2 text-navy focus:outline-none focus:border-red"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="text-xs font-mono truncate text-navy">{selectedAsset.mime}</div>
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-navy/60 mb-1">Size</label>
                  <div className="text-xs font-mono text-navy">{formatBytes(selectedAsset.size)}</div>
                </div>
              </div>
              ...
              <div className="space-y-4 pt-4 border-t border-gray-100">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full text-xs border border-gray-200 rounded-lg p-2.5 text-navy focus:outline-none focus:border-red"
